...
function renderToFiles() {
  ...
  const blog$ = renderComponentToFile(
    <Page title="Blog post listings | Kent C. Dodds" lastUpdated={lastUpdatedBlog} links={getLinks()}>
      <Blog posts={posts} />
    </Page>,
  ...
  const post$ = posts.map(({url, date, title, subtitle, default: Post}) => {
    return renderComponentToFile(
      <Page title={`${title} | Kent C. Dodds Blog`} lastUpdated={date} links={getLinks()}>
        <Post url={url} date={date} title={title} subtitle={subtitle} />
      </Page>,
