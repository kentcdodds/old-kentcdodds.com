...
author: 'Kent C. Dodds'
description:
  '_There are tons of syntaxes for function types and implementations, this lays
  them all out with simple examples_'
categories:
  - 'typescript'
keywords:
  - 'typescript'
...
function type as a type in itself. Any other time, use use `: ReturnType`.
...
// but it also comes at a cost that offsets any perf gains you get by going with a class over a regular object factor so...
```
...
I've not once used a generator in production code... So I'm going to have to
...
```typescript
const arrayWithFalsyValues = [1, undefined, 0, 2] // Array<number | undefined>
```

In regular JavaScript you can do:

```typescript
const arrayWithoutFalsyValues = arrayWithFalsyValues.filter(Boolean) // Array<number | undefined>
```
...
```typescript
const arrayWithoutFalsyValues = arrayWithFalsyValues.filter(typedBoolean) // Array<number>
```
