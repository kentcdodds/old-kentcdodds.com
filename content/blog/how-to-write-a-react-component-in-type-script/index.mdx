---
slug: 'how-to-write-a-react-component-in-type-script'
title: 'How to write a React Component in TypeScript'
date: '2021-03-04'
author: 'Kent C. Dodds'
description:
  "_There are plenty of ways to do it, here's how I recommend typing React
  Components_"
categories:
  - 'react'
  - 'typescript'
...
const Calculator: React.FC<CalculatorProps> = ({left, right, operator}) => {
  // implementation cliped for brevity
}
```
...
3. We have to use a function expression and can't use a function declarations.
...
if you want to dive deeper. Suffice it to say, don't use `React.FC` (or it's
longer alias `React.FunctionComponent`).
...
Ok, now this next bit has nothing to do with typing React components, but I
thought you'd find it interesting anyway, so skip ahead if you don't. Let's
...
But I'm not done... I'm not a fan of having to type each of the `operations`
objects. They're identically typed (by necessity due to the use case). The
`operations` object is really just a record of operation strings mapped to a
function that operates on two numbers. If we were to extract the type for those
functions into a single type, it would be:

```tsx
type OperationFn = (left: number, right: number) => number
```

So if we add a type annotation on our `operations` variable, then we don't have
to type each function individually. Let's try that:
...
At this point, we just need to decide which we dislike the least: If we type
every function individually, we deal with repetition in every function. If we
add a type annotation to the object, we have duplication between the property
names and the operators. Neither of these is more error prone, so it's really
just preference at this point. If I were reviewing a PR from you, I wouldn't
have an issue if you went with either approach.

Personally, I think I prefer the `type Operator` option because I just don't
like repeating the types in the functions.
