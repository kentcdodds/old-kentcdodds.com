---
slug: 'how-i-structure-express-apps'
title: 'How I structure Express apps'
date: '2020-07-11'
author: 'Kent C. Dodds'
description: '_The build/dev tools and scripts I use for Node backends._'
categories:
  - 'node'
keywords:
  - 'javascript'
...
Let's start in the `package.json`. Here are the relvant bits:
...
async errors will be propogated to your error handling middleware.
...
All it does is configure the logger based on the current environment and starts
the server. Most projects I've seen actually kick off the server in the
`src/index.js` file, but I prefer to take the logic for starting the server and
put it in a function because it makes it easier for testing.
...
**If this sounds interesting to you, then watch me teach you on
