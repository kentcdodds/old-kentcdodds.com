---
slug: 'how-to-write-a-type-narrowing-identity-function-in-typescript'
title: 'How to write a type narrowing identity function (TNIF) in TypeScript'
date: '2021-03-09'
author: 'Kent C. Dodds'
description:
  '_A handy advanced TypeScript pattern to increase your productivity._'
categories:
  - 'typescript'
keywords:
  - 'typescript'
...
## The type narrowing identity function
...
So here's where the type narrowing identity function comes in. By the way, "type
narrowing" describes a situation where you have one thing that could be many
possible values and you "narrow" it down to a few. Here's a simple example:

```tsx
const randomNumber = Math.random()
// typeof randomNumber === number
if (randomNumber === 0.5) {
  // typeof randomNumber === 0.5
}
```
...
So with those definitions out of the way, a "type narrowing identity function"
is a function which returns what it is given and also helps TypeScript narrow
its type. This is exactly what we want to do.

We can call it a **TNIF** (pronounced "tee niff"). Sure, let's go with that.
...
So the `createNumbers` is the type narrowing identity function. It returns the
...
missing feature of TypeScript. Our `createNumbers` type narrowing identity
function (er... "TNIF") is a workaround.

So here's what that workaround is:

**Type narrowing identity functions allow us to _not_ explicitly annotate our
...
Alrighty, so let's apply a TNIF to our original situation:
...
You may have noticed that we had two TNIFs in the previous section:
...
```tsx
const narrow = <Given, Inferred extends Given>(item: Inferred) => item

// @ts-expect-error Expected 2 type arguments, but got 1.(2558)
const numbers = narrow<Record<string, number>>({one: 1 /* etc. */})
```
...
```tsx
const narrow = <Given extends unknown>() => <Inferred extends Given>(
  item: Inferred,
) => item
...
```tsx
const createNumbers = narrow<Record<string, number>>()
const numbers = createNumbers({one: 1 /* etc. */})
```

But like, huh. Bummer.

Luckily, I don't find myself making TNIFs very often anyway and they aren't
