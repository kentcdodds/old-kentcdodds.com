...
1. Network requests (using a mocked version of `window.fetch`)
...
import {render, generate, fireEvent, screen} from 'til-client-test-utils'
...
test('login as an existing user', async () => {
  ...
  const {finishLoading} = render(<App />)
  ...
  await userEvent.type(getByLabelText(/username/i), fakeUser.username)
  await userEvent.type(getByLabelText(/password/i), fakeUser.password)

  // submit form
  const {post} = axiosMock.__mock.instance
  const token = generate.token(fakeUser)
  const fakeResponse = {data: {user: {...fakeUser, token}}}
  ...
  expect(window.fetch).toHaveBeenCalledWith(
    '/auth/login',
    object.containing({body: JSON.stringify(fakeUser)}),
  )
  expect(window.fetch).toHaveBeenCalledTimes(1)

  // assert the state of the world
  expect(window.localStorage.getItem('__auth_token__')).toBe(token)
...
between tests and automatically setting `window.fetch` to a
[mock function](https://jestjs.io/docs/en/mock-function-api) that has some good
default responses for requests made in my app.
