...
Let's take a look at another example here (this example is borrowed from my blog
post [Write fewer, longer tests](/blog/write-fewer-longer-tests)):
...
function Course({courseId}) {
  ...
  const {status, course, error} = state

  React.useEffect(() => {
    setState({status: 'pending', course: null, error: null})
    api.getCourseInfo(courseId).then(
      data => setState({status: 'fulfilled', course: data, error: null}),
      e => setState({status: 'rejected', course: null, error: e}),
    )
  }, [courseId])
  ...
  return (
    <>
      ...
      {status === 'fulfilled' ? <CourseInfo course={course} /> : null}
    </>
  )
}
...
Here's an example of a test suffering from this problem:
...
3. Put this assertion in another test (the premise of my article
   [Write fewer, longer tests](/blog/write-fewer-longer-tests))
...
out of keeping this assertion all by itself. To read more about this, please
read [Write fewer, longer tests](/blog/write-fewer-longer-tests).

## Other use cases for `act(...)`
